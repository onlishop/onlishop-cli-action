...
function getFilename() {
	...
	switch (osPlat) {
		case "win32": {
			return `onlishop-cli_Windows_${arch}.zip`;
		}
		case "darwin": {
			return `onlishop-cli_Darwin_${arch}.tar.gz`;
		}
		default: {
			return `onlishop-cli_Linux_${arch}.tar.gz`;
		}
	}
}
...
export async function install(version: string) {
	...
	exportVariable("ONLISHOP_CLI_VERSION", release.tag_name);
	process.env.ONLISHOP_CLI_VERSION = release.tag_name;
